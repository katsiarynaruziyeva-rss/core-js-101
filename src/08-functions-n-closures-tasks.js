...
function getPolynom(...args) {
  switch (args.length) {
    case 3: return (x) => args[0] * (x ** args[0]) + args[1] * x + args[2];
...
function logger(func, logFunc) {
  return (...args) => {
    const funcInfo = `${func.name}(${args.map((arg) => JSON.stringify(arg)).join(',')})`;
...
function partialUsingArguments(func, ...args1) {
  const partial = func.bind(null, ...args1);
  return function curried(...args) {
    if (args.length >= partial.length) {
...
//   const partial = fn.bind(null, ...args1);
//   return function curried(...args) {
//     if (args.length >= partial.length) {
//       return partial.apply(this, args);
//     }
